...
import com.epam.jwd.service.validator.api.OrderValidator;
...
public class OrderValidatorImpl implements OrderValidator {
    private static final Logger logger = LogManager.getLogger(OrderValidatorImpl.class);
    ...
    public void validate(OrderDto orderDto) throws ServiceException {
        if (Objects.isNull(orderDto)) {
            logger.info(ValidateException.PRICE_IS_EMPTY + OrderValidatorImpl.class);
    ...
    private Boolean checkNegative(Double pledge) throws ServiceException {
        if (pledge < MIN_NUMBER) {
            throw new ServiceException(ValidateException.ORDER + OrderValidatorImpl.class);
        }
        return true;
    }
