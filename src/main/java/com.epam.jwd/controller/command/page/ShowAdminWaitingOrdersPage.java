...
public enum ShowAdminWaitingOrdersPage implements Command {
    INSTANCE;

    private static final Logger logger = LogManager.getLogger(ShowAdminWaitingOrdersPage.class);

    private static final CommandResponse SUCCESS_RESPONSE = new CommandResponse() {
        @Override
        public String getPath() {
            return Constant.PATH_PAGE_ADMIN_WAITING_ORDERS;
        }
    ...
    public CommandResponse execute(CommandRequest request) {
        ...
        try {
            ...
            List<OrderDto> listOrderWaiting = orderService.getByStatus((int) Status.WAITING_PAYMENT.getId());
            Map<Integer, AccountDto> personMap = orderService.unionUserAndAccount((int) Status.WAITING_PAYMENT.getId());
            session.setAttribute("orderList", listOrderWaiting);
